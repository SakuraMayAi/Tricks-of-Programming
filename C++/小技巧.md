# 1 如何将一个缓冲区中的内容拷贝到 `string` 对象中

假设指向缓冲区的指针为 p，大小是 size，string 对象为 str。
...
```cpp
str.resize(size);
memcpy(&str[0], p + i, size);   // 使用 &str[0] 来获取字符串缓冲区的非 const 指针
```
...
```cpp
str.resize(size);
std::copy(p + i, p + i + size, str.begin());
```
